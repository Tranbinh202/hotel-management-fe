...
export default function AttendancePage() {
  ...
  const getLateStatusBadge = (lateMinutes: number) => {
    if (lateMinutes === 0) {
      return <Badge className="bg-green-500">Đúng giờ</Badge>
    } else if (lateMinutes <= 15) {
      return <Badge className="bg-yellow-500">Muộn {lateMinutes}p</Badge>
    } else {
      return <Badge className="bg-red-500">Muộn {lateMinutes}p</Badge>
    }
  }
  ...
  return (
    <div className="space-y-6">
              ...
              <TableBody>
                {attendanceData?.items.map((record) => (
                  <TableRow key={record.attendanceId}>
                    ...
                    <TableCell>{formatDate(record.shiftDate)}</TableCell>
                    <TableCell>{formatTime(record.checkIn)}</TableCell>
                    <TableCell>{formatTime(record.checkOut)}</TableCell>
                    <TableCell className="text-right font-medium">{record.workedHours.toFixed(2)}h</TableCell>
                    <TableCell className="text-right">{record.normalHours.toFixed(2)}h</TableCell>
                    <TableCell className="text-right text-orange-600 font-medium">
                      {record.overtimeHours.toFixed(2)}h
                    </TableCell>
                    <TableCell>{getLateStatusBadge(record.lateMinutes)}</TableCell>
      ...
      <EditAttendanceDialog
        ...
        onSubmit={(data) => {
          if (selectedRecord) {
            updateMutation.mutate({ id: selectedRecord.attendanceId, data })
...
function EditAttendanceDialog({
  ...
  const handleSubmit = () => {
    ...
    if (checkIn && record) {
      ...
      const checkInDate = new Date(record.shiftDate)
    ...
    if (checkOut && record) {
      ...
      const checkOutDate = new Date(record.shiftDate)
