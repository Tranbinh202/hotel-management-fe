import type { Account, ApiResponse, User, AccountSummary, UpdateAccountProfileDto } from "../types/api"
...
export const accountApi = {
  ...
  updateProfile: async (data: UpdateAccountProfileDto): Promise<ApiResponse<AccountSummary>> => {
    return await apiClient.put<ApiResponse<AccountSummary>>("/Account/profile", data)
  },
  ...
  getSummary: async (id: number): Promise<ApiResponse<AccountSummary>> => {
    return await apiClient.get<ApiResponse<AccountSummary>>("/Account/summary", {
      params: id !== 0 ? { id }: {},
    })
  },
